...
> ⚠ 勘误：hitTestBehavior 属性在部分仓颉 SDK 版本的通用属性中**未实现** — 
> Column/Row 等容器上调用会编译失败（'hitTestBehavior' is not a member），且 API 参考（cj-universal-attribute-*）中亦无此属性条目。
> 使用前先小样验证编译；悬浮层防触摸吞噬的实证替代方案：悬浮条作为 Stack 直接子组件的裸 Row（只占自身高度），用 .align(Alignment.Bottom) 定位，天然不阻挡底层触摸。
