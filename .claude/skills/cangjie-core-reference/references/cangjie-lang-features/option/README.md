...
> **注意**：`??` 的优先级低于比较运算符，混合使用时需加括号，详见 `../basic_data_type/README.md` §11.8「合并运算符 ??」。
...
`Option` 支持 `==`/`!=` 比较，但**裸 `None` 作比较操作数会撞名**
（万能导入场景报 `find multiple constructor 'None'`）— 必须显式消歧：

```cangjie
let m: ?String = Some("x")
if (m == Option<String>.None) { ... }   // ✓ 显式类型消歧
if (m == None<String>) { ... }          // ✓ 等价写法
// if (m == None) { ... }               // ✕ 万能导入下可能撞名
```

注意：这是 `Option` 的特例 — 自定义枚举默认**不**支持 `==`（无 Equatable），仍用 `match` 判断。
