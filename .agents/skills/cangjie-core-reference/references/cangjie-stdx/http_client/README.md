...
> ⚠ **鸿蒙应用选型**：应用内功能请使用 `ohos.net.http`（NetworkKit）— 要点：显式
> `expectDataType`、校验 `responseCode`、显式超时（API 查知识库 `HttpRequest request GET`）。
> 本篇的 `stdx.net.http` 可编译，但设备端行为未经验证，仅用于宿主侧工具代码。

## 1. 概述

- 依赖 `stdx.net.http`，关于扩展标准库 `stdx` 的配置用法，请参阅 `../config/README.md`
...
| OpenSSL 依赖 | HTTPS 需安装 OpenSSL 3，详见 `../tls/README.md` |
