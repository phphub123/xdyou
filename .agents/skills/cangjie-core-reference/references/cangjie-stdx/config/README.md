# 仓颉扩展标准库（stdx）项目配置与构建

> ⚠ **适用范围**：本篇仅适用于**独立仓颉工程**配置 stdx。
> 若工程由 Portage 迁移平台管理（工程根 CLAUDE.md 有「工程铁律」段），stdx **已由平台接线完毕**——
> 禁止按本篇下载 stdx 或改动 `cjpm.toml`（铁律禁改构建配置）；直接 `import stdx.*` 使用即可，
> 缺依赖报错时向平台反馈而不是自行配置。

## 0. 快速接线：本机已有 stdx 发行包（免下载）

> 本节为最小接线路径，优先于 §1–§3 的完整下载流程。

先找本机现成的 stdx 发行包（按序探测，找到即用）：

1. `~/.portage/stdx/<version>/`（迁移平台缓存，含 `linux_ohos_{aarch64,x86_64}_cjnative/` 两个 ABI 目录）
2. `~/.cangjie-stdx/`、工程内 `libs/stdx*/`
3. 都没有 → 走 §1 下载

**最小接线** = 给 `entry/cjpm.toml` 中两个 ohos target 的 `bin-dependencies.path-option` 各追加一条 stdx 动态库路径（无需 `[dependencies]` 段；Windows 宿主 target 不加）：

```toml
[target.aarch64-linux-ohos.bin-dependencies]
  path-option = ["${AARCH64_LIBS}", "${AARCH64_MACRO_LIBS}", "${AARCH64_KIT_LIBS}", "<stdx-root>/linux_ohos_aarch64_cjnative/dynamic/stdx"]

[target.x86_64-linux-ohos.bin-dependencies]
  path-option = ["${X86_64_OHOS_LIBS}", "${X86_64_OHOS_MACRO_LIBS}", "${X86_64_OHOS_KIT_LIBS}", "<stdx-root>/linux_ohos_x86_64_cjnative/dynamic/stdx"]
```

- `<stdx-root>` 替换为实际路径（正斜杠），如 `C:/Users/<you>/.portage/stdx/1.1.0.1`。
- 只追加路径项，**不动** path-option 里已有的 `${...}` 变量与该文件其他内容。
- 接线后 `import stdx.encoding.json.*` 等即可编译；无需改 module.json5。
