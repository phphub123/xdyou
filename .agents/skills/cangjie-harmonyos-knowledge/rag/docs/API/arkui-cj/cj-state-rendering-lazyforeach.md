...
> ⚠ 勘误：**调用处不要写显式泛型** `ForEach<String>(...)` —— 宏展开会把 `<` `>`
> 解析成链式比较（报 `comparison operators cannot be chained`，或只剩一句不透明的
> `macro evaluation has failed for macro call 'Component'`）。**省略泛型靠推断**即可编译通过：
> `ForEach(items, itemGenerator: { item: String, idx: Int64 => Text(item) })`。LazyForEach 同理。
